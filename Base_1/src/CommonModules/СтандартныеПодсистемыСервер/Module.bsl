...
Процедура ПередЗапускомПрограммы()
	...
	МинимальнаяВерсияПлатформы = " 8.3.15.1830";
...
Функция ТребуетсяУстановкаРегиональныхНастроекИнформационнойБазы()
	...
	УстановитьПривилегированныйРежим(Истина);
	// БК 3.0. При использовании работы в модели сервиса, в случае использования старых настроек, ОсновнойЯзык будет незаполнен
	//ТребуетсяУстановкаОсновногоЯзыка = ?(Метаданные.Языки.Количество() > 1, Не ЗначениеЗаполнено(Константы.ОсновнойЯзык.Получить()), Ложь);
	ТребуетсяУстановкаОсновногоЯзыка = Ложь;
