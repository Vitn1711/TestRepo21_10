...
Функция ЭтоКодЛогистическойУпаковки_ИСМП(КодМаркировки, ЗначениеSSCC = Неопределено) Экспорт
	...
	Если СтрДлина(КодМаркировки) = 18
		И ПечатьКодовМаркировкиИСМПТККлиентСервер.КонтрольноеЧислоSSCC(
...
Функция КодМаркировкиБезСкобок(КодМаркировки) Экспорт
	...
	ЧтениеШтрихкода = ПечатьКодовМаркировкиИСМПТККлиентСервер.ПараметрыШтрихкодаGS1СоСкобками(КодМаркировки);
	Если ЧтениеШтрихкода.Результат <> Неопределено Тогда
		Возврат ПечатьКодовМаркировкиИСМПТККлиентСервер.ШтрихкодGS1(ЧтениеШтрихкода.Результат, Ложь, "");
	КонецЕсли;
...
Функция ШтрихкодВФорматеGS1(Штрихкод) Экспорт
	...
	ЧтениеШтрихкода = ПечатьКодовМаркировкиИСМПТККлиентСервер.ПараметрыШтрихкода(Штрихкод);
	Если Не ЧтениеШтрихкода.Результат = Неопределено Тогда
		...
		Если ЧтениеШтрихкода.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодовИСМПТК.SSCC") Тогда
			...
			ШтрихкодGS1 = ПечатьКодовМаркировкиИСМПТККлиентСервер.ШтрихкодSSCC(ЧтениеШтрихкода.Результат, Истина);
			Возврат ШтрихкодGS1;
		...
		ИначеЕсли ЧтениеШтрихкода.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодовИСМПТК.GS1_128")
			...
			ШтрихкодGS1 = ПечатьКодовМаркировкиИСМПТККлиентСервер.ШтрихкодGS1(ЧтениеШтрихкода.Результат, Истина);
			Возврат ШтрихкодGS1;
...
Функция ШтрихкодВBase64(Штрихкод) Экспорт
	...
	ШтрихкодВBase64 = ИнтеграцияИСМПТККлиентСервер.ДвоичныеДанныеBase64(ДвоичныеДанные);
