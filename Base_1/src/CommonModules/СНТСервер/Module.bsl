...
Функция РазрешенныеДействияПоСтатусамСНТ(Знач Направление, Знач ВидОперацииСНТ = Неопределено, Знач ОбъектСНТ = Неопределено, Знач ВидПеремещенияСНТ = Неопределено) Экспорт
	...
	Если Направление = Перечисления.НаправленияСНТ.Исходящий Тогда	
		...
		Если ВидОперацииСНТ = Перечисления.ВидыОперацийСНТ.Ввоз 
			...
			ИЛИ (ОбъектСНТ.ПоставщикМалаяТорговаяТочка И ОбъектСНТ.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров
			...
			И НЕ ОбъектСНТ.ЕстьДругиеТоварыЦифроваяМаркировка И НЕ ОбъектСНТ.ЕстьТоварыЭК)
		...
		Если (ОбъектСНТ.ПоставщикМалаяТорговаяТочка И ОбъектСНТ.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров
			И НЕ ОбъектСНТ.ЕстьЭтиловыйСпирт И НЕ ОбъектСНТ.ЕстьВиноматериал И НЕ ОбъектСНТ.ЕстьАлкоголь И НЕ ОбъектСНТ.ЕстьПивоПивныеНапитки
			И НЕ ОбъектСНТ.ЕстьНефтепродукты И НЕ ОбъектСНТ.ЕстьБиотопливо И НЕ ОбъектСНТ.ЕстьТабачныеИзделия 
	...
	Иначе
		...
		Если ПроверитьСтатусНеТребуетПодтверждения(ОбъектСНТ) // условия когда не требуется подтверждение документа "СНТ"
			И НЕ (ОбъектСНТ.ПолучательМалаяТорговаяТочка И ОбъектСНТ.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров) Тогда
			СоответствиеДоступностиДействия.Вставить(Перечисления.СтатусыСНТ.НеПросмотрен				, Ложь);
			СоответствиеДоступностиДействия.Вставить(Перечисления.СтатусыСНТ.Доставленный				, Ложь);
		КонецЕсли;
		...
		Если ПроверитьСтатусНеТребуетПодтверждения(ОбъектСНТ) Тогда // условия когда не требуется подтверждение документа "СНТ"
			Если ((ОбъектСНТ.ПолучательРозничныйРеализатор И ОбъектСНТ.Дата < СНТСерверПереопределяемый.ДатаВводаМТТ()) 
				ИЛИ ОбъектСНТ.ПолучательМалаяТорговаяТочка ИЛИ ПроверитьСтатусНеТребуетПодтверждения(ОбъектСНТ)) Тогда
				СоответствиеДоступностиДействия.Вставить(Перечисления.СтатусыСНТ.НеПросмотрен			, Ложь);
				СоответствиеДоступностиДействия.Вставить(Перечисления.СтатусыСНТ.Доставленный			, Ложь);
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьСтатусНеТребуетПодтверждения(ОбъектСНТ) Экспорт
	...
	// Получатель «Малая торговая точка», "Розничный реализатор" и дата выписки до 5.02.2022 для РР и заполнен раздел G1
	Если ((ОбъектСНТ.ПолучательРозничныйРеализатор И ОбъектСНТ.Дата < СНТСерверПереопределяемый.ДатаВводаМТТ()) ИЛИ ОбъектСНТ.ПолучательМалаяТорговаяТочка
			ИЛИ (ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Входящий И ОбъектСНТ.ПоставщикМалаяТорговаяТочка И ОбъектСНТ.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров))
		И НЕ ОбъектСНТ.ЕстьЭтиловыйСпирт И НЕ ОбъектСНТ.ЕстьВиноматериал И НЕ ОбъектСНТ.ЕстьАлкоголь И НЕ ОбъектСНТ.ЕстьПивоПивныеНапитки
		И НЕ ОбъектСНТ.ЕстьНефтепродукты И НЕ ОбъектСНТ.ЕстьБиотопливо И НЕ ОбъектСНТ.ЕстьТабачныеИзделия 
		И НЕ ОбъектСНТ.ЕстьДругиеТоварыЦифроваяМаркировка И НЕ ОбъектСНТ.ЕстьТоварыЭК Тогда
