...
Функция ТекстЗапросаНаПолучениеИнформацииОКонтактах(ВключатьEmail)
	...
	|	"""" КАК НаименованиеВладельца " + ПолучитьСтрокуПроАдресДляЗапросаИнформацииОКонтактах(ВключатьEmail,"Пользователи",Истина) + "
	|ИЗ
...
Функция КонтактыПоДоменномуАдресу(Знач ИмяДомена)
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если ОписаниеКонтакта.ИскатьПоДомену Тогда
			...
			|	ТаблицаКонтактнаяИнформация.Объект.Наименование КАК Наименование,
			|	ВЫРАЗИТЬ(ТаблицаКонтактнаяИнформация.Представление КАК СТРОКА(100)) КАК Представление,
			|	" + ?(ОписаниеКонтакта.ЕстьВладелец,"ТаблицаКонтактнаяИнформация.Объект.Владелец.Наименование ","""""") + ?(ПерваяТаблица," КАК НаименованиеВладельца ","") +" 
			|ИЗ
			|	РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
			|ГДЕ
			|	ТаблицаКонтактнаяИнформация.Объект ССЫЛКА Справочник." + ОписаниеКонтакта.Имя + "
