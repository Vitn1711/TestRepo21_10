...
Функция ШтрихкодGS1(ИспользуемыеИдентификаторы, УстанавливатьСкобки = Истина, СимволFNC1 = "") Экспорт
	...
	Иначе
		...
		СвойстваИдентификаторов = ПечатьКодовМаркировкиИСМПТККлиентСерверПовтИсп.СвойстваКлючейИдентификаторовПрименения();
			...
			ИначеЕсли ТипЗнч(Значение) = Тип("Число") Тогда
				...
				Если ЗначениеЗаполнено(СвойстваИдентификатора.ДополнительныйПараметрИмя)
				   И СвойстваИдентификатора.ДополнительныйПараметрИмя = ВРЕГ("ЧислоЗнаковПослеЗапятой") Тогда
					...
					ФорматнаяСтрокаЧисла = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ФорматнаяСтрокаЧисла, ДопПараметр);
				КонецЕсли;
...
Функция ПараметрыШтрихкодаSSCC(Знач Штрихкод) Экспорт
	...
	Если Не ОшибкаЧтения Тогда
		...
		Если ДлинаСтроки <> 18 Тогда
			...
			ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ДлинаСтроки, 18);
		...
		Иначе
			...
			Результат.Вставить("ЦифраРасширения",    СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(
				Сред(НепрочитаннаяЧастьШК, 1, 1)));
			Результат.Вставить("ПрефиксКомпанииGS1", СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(
				Сред(НепрочитаннаяЧастьШК, 2, 9)));
			Результат.Вставить("СерийныйНомерSSCC",  СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(
				Сред(НепрочитаннаяЧастьШК, 11, 7)));
			Результат.Вставить("КонтрольноеЧисло",   СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(
...
Функция ПараметрыШтрихкодаGS1(Знач Штрихкод, ПараметрыШтрихкода = Неопределено) Экспорт
	...
	Если НомерПозиции = 1 Тогда
		...
		ПараметрыШтрихкода.ТекстОшибки  = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(
...
Функция КонтрольноеЧислоSSCC(Штрихкод) Экспорт
	...
	Для НомерПозиции = 1 По Позиций Цикл
		Цифры.Добавить(СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(Сред(Штрихкод, НомерПозиции, 1)));
	КонецЦикла;
...
Функция ПараметрЗначенияИдентификатораПрименения(СвойстваИдентификатораПрименения, КлючИдентификатора, ДополнительныйПараметр, ЗначениеСтрокой, ТекстОшибки = "")
	...
	ПараметрЗначенияИдентификатора.Вставить("ДополнительныйПараметр", 
		СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(ДополнительныйПараметр));
	...
	ИначеЕсли ТипЗначенияИдентификатора.СодержитТип(Тип("Число")) Тогда
		Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЗначениеСтрокой) Тогда
			...
			ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ЗначениеСтрокой);
			
			ПараметрЗначенияИдентификатора = Неопределено;
			Возврат ПараметрЗначенияИдентификатора;
		КонецЕсли;
		Если СвойстваИдентификатораПрименения.ДополнительныйПараметрИмя = ВРЕГ("ЧислоЗнаковПослеЗапятой") Тогда
			ЗначениеДопПараметра = СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(ДополнительныйПараметр);
			...
			КонецЕсли;
			ЗначениеПараметра = СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(СтроковоеЗначениеПараметра);
		Иначе
			СтроковоеЗначениеПараметра = ЗначениеСтрокой;
			ЗначениеПараметра = СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(СтроковоеЗначениеПараметра);
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ЗначениеПараметра)
		...
		ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(
			ТекстОшибки,
...
Функция СтрокаGS1ВДату(Знач Значение, Знач ОписаниеТиповДаты, ТекстОшибки = "")
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		...
		ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, Значение);
		Возврат Неопределено;
	КонецЕсли;
	
	Если ЧастьДаты = ЧастиДаты.Дата Тогда
		Если СтрДлина(Значение) = 6 Тогда
			Год = СлужебныеПроцедурыИФункцииИСМПТККлиентСервер.СтрокаВЧисло(Лев(Значение, 2));
