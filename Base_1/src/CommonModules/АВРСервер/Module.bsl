...
Функция ПодготовитьТаблицыДанныхДляЗаполненияАВР(ПараметрыЗаполнения) Экспорт 
	
	ТекстЗапроса = АВРСерверПереопределяемый.ПодготовитьТекстЗапросаДляЗаполненияАВР(ПараметрыЗаполнения);
	...
	Для Каждого КлюИЗначение Из ПараметрыЗаполнения Цикл
		Запрос.УстановитьПараметр(КлюИЗначение.Ключ, КлюИЗначение.Значение);
	КонецЦикла;	
	
	РезультатЗапроса  = Запрос.ВыполнитьПакет();
	
	ТаблицыДанныхДляЗаполненияАВР = Новый Структура;	
	ТаблицыДанныхДляЗаполненияАВР.Вставить("ТаблицаРеквизитов",	РезультатЗапроса[0].Выгрузить());
	ТаблицыДанныхДляЗаполненияАВР.Вставить("Услуги",			РезультатЗапроса[1].Выгрузить());
	
	Возврат ТаблицыДанныхДляЗаполненияАВР;
...
Функция ПодготовитьДанныеДляЗаполненияАВР(ПараметрыЗаполнения) Экспорт
	
	ТаблицыДанныхДляЗаполненияАВР = ПодготовитьТаблицыДанныхДляЗаполненияАВР(ПараметрыЗаполнения);
	Если ТаблицыДанныхДляЗаполненияАВР = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ДанныеДляЗаполнения = Новый Структура;
	
	ТаблицаРеквизитов 	  = ТаблицыДанныхДляЗаполненияАВР.ТаблицаРеквизитов;
	ДанныеДляЗаполнения.Вставить("Реквизиты", ТаблицаРеквизитов);
	
	Поставщик = ТаблицаРеквизитов[0].Поставщик;	
	Если ЗначениеЗаполнено(Поставщик) Тогда
		СтруктураДанныхПоставщики = АВРСерверПереопределяемый.ПолучитьРеквизитыОрганизации(Поставщик, "Поставщик", ТаблицаРеквизитов[0]);
	Иначе
		СтруктураДанныхПоставщики = Неопределено;
	КонецЕсли;	
	ДанныеДляЗаполнения.Вставить("СтруктураДанныхПоставщики", СтруктураДанныхПоставщики);
	
	Получатель = ТаблицаРеквизитов[0].Контрагент;
	Если ЗначениеЗаполнено(Получатель) Тогда
		СтруктураДанныхПолучатели = АВРСерверПереопределяемый.ПолучитьРеквизитыКонтрагента(ТаблицаРеквизитов[0].Контрагент, "Получатель", ТаблицаРеквизитов[0]);
	Иначе
		СтруктураДанныхПолучатели = Неопределено;
	КонецЕсли;
	ДанныеДляЗаполнения.Вставить("СтруктураДанныхПолучатели", СтруктураДанныхПолучатели);
	
	ДанныеДляЗаполнения.Вставить("Услуги", ТаблицыДанныхДляЗаполненияАВР.Услуги);
