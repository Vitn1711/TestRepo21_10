...
Процедура СвернутьСтрокиРасчетаОтОбратного(ТЗНачисления,
	...
	Пока Н >= 0 Цикл
		...
		Если ПредыдущийВидРасчета = Неопределено Тогда
			// запоминаем вид расчета
			ПредыдущийВидРасчета = Строка.ВидРасчета;
		ИначеЕсли ПредыдущийВидРасчета <> Строка.ВидРасчета Тогда
			// повторяется вид расчета
			Сообщить("" + Строка.Сотрудник + ": в одном документе нельзя рассчитывать более одного вида расчета ""от обратного""!", СтатусСообщения.Важное);
		КонецЕсли;
