	...
	Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл
		...
		Для Каждого ТекУчет Из СтруктураУчет Цикл
			...
			Для Каждого СтрокаПартии Из СтрокиСписанныхПартий Цикл
				...
				КоэффициентПересчета = СтрокаПартии.Количество / СтрокаТаблицы.Количество;
				СтрокаПартии.СебестоимостьВал = Окр(СтрокаТаблицы.СуммаВал * КоэффициентПересчета, 2);
				СтрокаПартии.Себестоимость    = Окр(СтрокаТаблицы.Сумма * КоэффициентПересчета, 2);
			...
			Если ИтогоСебестоимостьВал > 0 И ИтогоСебестоимостьВал <> СтрокаТаблицы.СуммаВал Тогда 
				...
				СтрокаСМаксСуммой.СуммаСписанияВал = СтрокаПартии.СебестоимостьВал;
			КонецЕсли;
			...
			Если ИтогоСебестоимость > 0 И ИтогоСебестоимость <> СтрокаТаблицы.Сумма Тогда 
				...
				СтрокаСМаксСуммой.СуммаСписания    = СтрокаПартии.Себестоимость;
				СтрокаСМаксСуммой.СуммаСписанияНУ  = СтрокаПартии.Себестоимость;
			КонецЕсли;
