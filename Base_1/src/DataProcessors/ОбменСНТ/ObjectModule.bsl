...
Процедура ДокументСНТ_ОбработкаПроведения(ОбъектСНТ, Отказ, РежимПроведения) Экспорт
	...
	Если ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий И (ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз ИЛИ ОбъектСНТ.ПоставщикМалаяТорговаяТочка)
...
Функция НайтиСсылкиПоЗначениямРеквизитов(Знач ЗначенияРеквизитов, Знач ОбъектМетаданных, Знач ИдентификационныйНомер = Неопределено) Экспорт
	...
	Если ИдентификационныйНомер <> Неопределено И ИмяОбъекта <> "СопоставлениеСНТиФНО" Тогда
		...
		//|	И НЕ СНТ.ПоставщикМалаяТорговаяТочка
...
Процедура РазделитьМассивПоНаправлениямСНТ(МассивСНТ, МассивВходящихСНТ, МассивИсходящихСНТ)
	...
	Для Каждого СНТ из МассивСНТ Цикл
		Если СНТ.Направление = Перечисления.НаправленияСНТ.Входящий
				И НЕ (СНТ.ПоставщикМалаяТорговаяТочка И СНТ.ТипСНТ =Перечисления.ТипыСНТ.ВозвратТоваров И СНТ.ТоварыВС.Количество() > 0
		...
		ИначеЕсли СНТ.Направление = Перечисления.НаправленияСНТ.Входящий 
				И (СНТ.ПоставщикМалаяТорговаяТочка И СНТ.ТипСНТ =Перечисления.ТипыСНТ.ВозвратТоваров И СНТ.ТоварыВС.Количество() > 0
						И СНТ.ЭтиловыйСпирт.Количество() = 0 И СНТ.Виноматериал.Количество() = 0 И СНТ.ПивоПивныеНапитки.Количество() = 0
						И СНТ.АлкогольнаяПродукцияКромеПива.Количество() = 0 И СНТ.ДанныеПоНефтепродуктам.Количество() = 0 И СНТ.ДанныеПоБиотопливу.Количество() = 0
						И СНТ.ТабачнаяПродукция.Количество() = 0 И СНТ.ДругиеТоварыЦифроваяМаркировка.Количество() = 0 И СНТ.ТоварыЭкспортныйКонтроль.Количество() = 0) Тогда
		...
		Для каждого ТабличнаяЧасть Из ПроверяемыеТабличныеЧасти Цикл
			Если ОбъектСНТ[ТабличнаяЧасть].Количество() > 0 Тогда
				Для Каждого Товар Из ОбъектСНТ[ТабличнаяЧасть] Цикл
					Если НЕ ЗначениеЗаполнено(Товар.ДополнительныйИдентификатор) И Товар.ИдентификаторТовара <> "1" Тогда //ИД товара должен быть заполнен, либо если это обыный товар то должен иметь значение = 1
...
Функция ПодготовитьПараметрыПроведенияСНТ(ДокументСсылка, Отказ) Экспорт	
	...
	Если ЗначениеЗаполнено(ДокументСсылка) Тогда
		Если (ДокументСсылка.Направление = Перечисления.НаправленияСНТ.Исходящий 
			И (ДокументСсылка.ПоставщикМалаяТорговаяТочка ИЛИ ДокументСсылка.ПоставщикРозничныйРеализатор)
			И ДокументСсылка.ТипСНТ =Перечисления.ТипыСНТ.ВозвратТоваров 
			И ДокументСсылка.ТоварыВС.Количество() = 0)
			ИЛИ (ДокументСсылка.Направление = Перечисления.НаправленияСНТ.Входящий
	...
	Если (Реквизиты.Направление = Перечисления.НаправленияСНТ.Входящий И (НЕ(Реквизиты.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И Реквизиты.ПоставщикМалаяТорговаяТочка) ИЛИ ВозвратМТТАкциз)) //НЕ(Реквизиты.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И Реквизиты.ПоставщикМалаяТорговаяТочка И ВозвратМТТАкциз) //И НЕ ВозвратМТТАкциз
		ИЛИ 
		(Реквизиты.Направление = Перечисления.НаправленияСНТ.Исходящий И (Реквизиты.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз ИЛИ (Реквизиты.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И Реквизиты.ПоставщикМалаяТорговаяТочка И НЕ ВозвратМТТАкциз))) Тогда
...
Процедура СоздатьОбновитьИсточникиПроисхождения(ОбъектСНТ, ИмяТабличнойЧасти, НомерТабличнойЧасти)
	...
	Для Каждого СтрокаТовар Из ОбъектСНТ[ИмяТабличнойЧасти] Цикл
		...
		Если ТипЗнч(ОбъектСНТ) = Тип("ДокументОбъект.СНТ") Тогда
			...
			Если ИмяТабличнойЧасти <> "Товары" И (ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Входящий ИЛИ ОбъектСНТ.ВидВвоза = Перечисления.ВидыВвозаСНТ.Импорт
