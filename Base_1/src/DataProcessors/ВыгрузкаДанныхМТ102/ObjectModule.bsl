	...
	Если ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеПенсионныхВзносов ИЛИ
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоПлатежномуПоручению.ГЦВПСсылка) Тогда
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоПлатежномуПоручению.ГЦВПНаименованиеПолное) Тогда
				...
				ТекстСообщения = НСтр("ru='Не указано полное наименование для ГЦВП %1'");
			    ...
			    ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,ВыборкаПоПлатежномуПоручению.ГЦВПСсылка),
														 ВыборкаПоПлатежномуПоручению.ГЦВПСсылка);
				Отказ = Истина;										 
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоПлатежномуПоручению.ГЦВПИдентификационныйНомер) Тогда
				...
				ТекстСообщения = НСтр("ru='Не указан БИН/ИИН для ГЦВП %1'");
			...
			ИначеЕсли СтрДлина(СокрЛП(ВыборкаПоПлатежномуПоручению.ГЦВПИдентификационныйНомер)) < 12 Тогда
				...
				ТекстСообщения = НСтр("ru='Длина БИН/ИИН для ГЦВП %1 меньше 12 знаков'");
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоПлатежномуПоручению.ГЦВППризнакРезидентства) ИЛИ
				...
				ТекстСообщения = НСтр("ru='Для ГЦВП %1 не заполнен КБе (должен быть из 2 знаков)'");
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоПлатежномуПоручению.ГЦВПБанковскийСчет) Тогда
				ТекстСообщения = НСтр("ru='Не указан номер расчетного счета ГЦВП %1'");
			    ...
			    ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,ВыборкаПоПлатежномуПоручению.ГЦВПБанковскийСчет),
														 ВыборкаПоПлатежномуПоручению.ГЦВПБанковскийСчетСсылка);
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоПлатежномуПоручению.ГЦВПБИК) Тогда
				...
				ТекстСообщения = НСтр("ru='Для ГЦВП %1 не указан БИК банка'");
			    ...
			    ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,ВыборкаПоПлатежномуПоручению.ГЦВПБанкСсылка),
														 ВыборкаПоПлатежномуПоручению.ГЦВПБанкСсылка);				
...
Процедура СформироватьСтрокуРаботникаФайла(ФайлМТ102, ВыборкаПоСпискуРаботников, НомерСтроки, ПолеPERIODвЗаголовке, ВключатьПолеFT,ВидОперации)
	...
	Если ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеПенсионныхВзносов 
		ИЛИ ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеСоциальныхОтчислений И НЕ ПолеPERIODвЗаголовке Тогда
		СтрокаФайла = СтрокаФайла + "
...
Процедура СформироватьФайлОтчислений(ФормироватьОтдельныеФайлы,ПолеPERIODвЗаголовке,ВключатьПолеPSO, ВключатьПолеPRT,ВключатьПолеFT, НомерФайла, СтруктураПередаваемыхФайлов, УникальныйИдентификатор,ТекстВыгрузки = Неопределено, ИспользоватьПолныйПуть)
	...
	Пока ВыборкаПоСпискуРаботников.Следующий() Цикл
		...
		СформироватьСтрокуРаботникаФайла(ФайлМТ102, ВыборкаПоСпискуРаботников, НомерСтроки, ПолеPERIODвЗаголовке, ВключатьПолеFT, ВидОперации);
