...
Процедура ЗаполнитьПраваПользователей()
	...
	//|		В БК 3.0 КИ не из БСП
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК ПочтовыеАдреса
	|		ПО Пользователи.Ссылка = ПочтовыеАдреса.Объект
	|			И (ПочтовыеАдреса.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
	|			И (ПочтовыеАдреса.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.СлужебныйАдресЭлектроннойПочтыПользователя))
	|ГДЕ
