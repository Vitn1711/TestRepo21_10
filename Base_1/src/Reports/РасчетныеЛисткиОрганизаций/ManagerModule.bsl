				...
				Пока ВыборкаПоФИО.Следующий() Цикл
					...
					Пока ВыборкаПоФЛ.Следующий() Цикл
						...
						Если ВыборкаПоФЛ.ОППВСНачалаГода <> 0 ИЛИ ВыборкаПоФЛ.ОППВЗаМесяц <> 0 Тогда
							ОбластьОППВ.Параметры.ОППВСНачалаГода = Формат(ВыборкаПоФЛ.ОППВСНачалаГода, "ЧЦ=15; ЧДЦ=2");
							ОбластьОППВ.Параметры.ОППВЗаМесяц = Формат(ВыборкаПоФЛ.ОППВЗаМесяц, "ЧЦ=15; ЧДЦ=2");
							ВременныйТабличныйДокумент.Вывести(ОбластьОППВ);
						КонецЕсли;
