...
Процедура СформироватьСводНачисленийУдержаний(ТабличныйДокумент, ПараметрыОтчета)
	...
	ОбластьООСМС	= Макет.ПолучитьОбласть("ООСМС");
	...
	СтруктураПустогоПодвала = Новый Структура(
		...
		|СоциальныйНалог, СоциальныеОтчисления,
	...
	Пока ЕстьДанные Цикл
		...
		// Очистим суммы ООСМС
		ОбластьОППВ.Параметры.ООСМС = 0;
		ОбластьООСМС.Параметры.ООСМС = 0;
		...
		Пока ВыборкаПоПериоду.Следующий() Цикл
			...
			Иначе		
				...
				Иначе
					...
					ОбластьПодвал.Параметры.СоциальныйНалог      = ОбластьПодвал.Параметры.СоциальныйНалог      + ВыборкаПоПериоду.СоциальныйНалог;
					ОбластьПодвал.Параметры.СоциальныеОтчисления = ОбластьПодвал.Параметры.СоциальныеОтчисления + ВыборкаПоПериоду.СоциальныеОтчисления;
					
					// Данные ОППВ
					ОбластьОППВ.Параметры.ОППВ = ОбластьОППВ.Параметры.ОППВ + ВыборкаПоПериоду.ОППВ;
					
					// Данные ООСМС
					ОбластьОППВ.Параметры.ООСМС = ОбластьОППВ.Параметры.ООСМС + ВыборкаПоПериоду.ООСМС;
					ОбластьООСМС.Параметры.ООСМС = ОбластьООСМС.Параметры.ООСМС + ВыборкаПоПериоду.ООСМС;
				КонецЕсли;
		...
		Иначе
			ТабличныйДокумент.Вывести(ОбластьООСМС);
		КонецЕсли;
