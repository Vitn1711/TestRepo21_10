...
Процедура ВыгрузитьВXML(Команда)
	...
	АдресВременногоХранилища = ВыгрузитьВXMLНаСервере(Истина);
	...
	ДиалогСохраненияФайла.ПолноеИмяФайла = КодФормы + " " + СтруктураРеквизитовФормы.мГод + "кв" + СтруктураРеквизитовФормы.мКвартал + ".xml";
...
Процедура ЗаполнитьПанель(ПризнакОсновнойФормы, ПризнакМногострочности)
	...
	Элементы.ФормаВыгрузитьВExcel.Видимость 	= ПризнакМногострочности И КодФормы <> "Форма300_03";
		...
		Выборка05.Сбросить();
	 	...
	 	Пока Выборка05.Следующий() Цикл
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				//Элемент.Ф300_05_B             = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Контрагент, "Наименование");
				...
				Если ЗначениеЗаполнено(Договор) Тогда
					...
					Элемент.Ф300_05_E               = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Договор, "НомерДоговора");
					Элемент.Ф300_05_E_Расшифровка   = Договор;

					Элемент.Ф300_05_E2              = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Договор, "ДатаДоговора");
...
Процедура ВыгрузитьВExcel(Команда)
	...
	ВыбратьИПередатьФайлШаблонаНаСервер(ПараметрыВыгрузкиВExcel.ИмяФайлаШаблона, КодФормы);
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьИПередатьФайлШаблонаНаСервер(ИмяФайлаШаблона, ИмяФормы)
	
	РасширениеПодключено = ПодключитьРасширениеРаботыСФайлами();
	Если НЕ РасширениеПодключено Тогда 
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ТекстПредложения", НСтр("ru = 'Для выбора шаблона Excel 
	...
	ДиалогОткрытияФайла.ПолноеИмяФайла = ИмяФайлаШаблона;
	...
	Оповещение = Новый ОписаниеОповещения("ВыбратьИПередатьФайлШаблонаНаСерверЗавершение", ЭтотОбъект, Новый Структура);	
	Оповещение.ДополнительныеПараметры.Вставить("ИмяФормы", ИмяФормы);	
	
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		НачатьПомещениеФайла(Оповещение, , ДиалогОткрытияФайла.ПолноеИмяФайла, Ложь, УникальныйИдентификатор);
	КонецЕсли;
...
//
Процедура ВыбратьИПередатьФайлШаблонаНаСерверЗавершение(Знач Успешно, Знач АдресФайлаШаблона, Знач ВыбранноеИмяФайла, Знач ДополнительныеПараметры) Экспорт
	
	Если Не Успешно Тогда
		Возврат;
	КонецЕсли;
	
	// Заполним шаблон Excel по указанному адресу
	ПараметрыВыгрузкиВExcel = ПолучитьПараметрыВыгрузкиВExcel(ДополнительныеПараметры.ИмяФормы);
...
#КонецЕсли
	...
	// Сообщить о завершении выгрузки и показать файл
