...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	КонецЦикла;		
	// 36. Итого капитал(сумма строк с 37 по 42)
	Для НомерПериода = 1 по 2 Цикл
		Итого = 0;
		Для Счетчик  = 37 по 42 Цикл
			НомерСтроки = "s_" + Формат(Счетчик,"ЧЦ=2; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
	...
	// 43. Итого капитал и обязательства (стр.22 + стр.36)								
	Для НомерПериода = 1 по 2 Цикл
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_43_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;	
