...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	...
	Если СтрокаОтчеты.СКД Тогда
		...
		//ПараметрПериодРассылки = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПараметр(ПараметрыОтчета.ПользовательскиеНастройкиКД, "ПериодОтчета");
		...
		Если НЕ РежимВариантаОтчета Тогда
			...
			КонецЦикла;
			//Для Каждого ЭлементНастроек Из ПараметрыОтчета.ПользовательскиеНастройкиКД.Элементы Цикл
			//	Если ЭлементНастроек = ПараметрПериодРассылки Тогда
			//		Продолжить;
			//	КонецЕсли;
			//	ЭлементНастроек.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
		...
		Иначе
			...
			//	КонецЕсли;
			//КонецЦикла;
			//ПараметрПериодРассылки = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПараметр(ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки, "ПериодРассылки");
			//ПользовательскиеНастройкиНовые = Новый ПользовательскиеНастройкиКомпоновкиДанных;
			////НовыйЭлементНастроек = ПользовательскиеНастройкиНовые.Элементы.Добавить(ПараметрПериодРассылки);
			//ПараметрыОтчета.КомпоновщикНастроекКД.ЗагрузитьПользовательскиеНастройки(Новый ПользовательскиеНастройкиКомпоновкиДанных);
			////ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки.Элементы.Очистить();
		КонецЕсли;
...
Процедура ОбработкаОповещенияНаСервере(Параметр)
	...
	//РеквизитФормыВЗначение("Объект").Отчеты[0].Отчет
	//СтрокаОтчеты.Отчет
	
	//Для Каждого СтрокаОтчеты Из РассылкаОбъект.Отчеты Цикл
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		Если СтрокаОтчеты.ПолноеИмя = Параметр.ПолноеИмя Тогда
			...
			//ЗаписатьНастройкиСтрокиОтчеты(СтрокаОтчеты.ПолучитьИдентификатор());
		//Модифицированность = Истина;
