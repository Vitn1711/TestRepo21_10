...
Процедура ЗагрузитьКлассификаторИзКаталога(Знач ФайлДанных)
	...
	РезультатАнализа = АдресныйКлассификаторКлиентСервер.НайтиФайл(ФайлДанных, "");
	Если НЕ РезультатАнализа.Существует <> Неопределено Тогда
		// Не хватает файлов для загрузки по указанным режимам
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Указанный файл не найден'"), , "АдресЗагрузки");
		Возврат;
	КонецЕсли;
	...
	РезультатАнализа.Вставить("КодыОбластей",    КодыОбластей);
	РезультатАнализа.Вставить("ЕстьВсеФайлы",    Истина);
	РезультатАнализа.Вставить("Ошибки",          Неопределено);
	РезультатАнализа.Вставить("Кодировка",       Кодировка);
	РезультатАнализа.Вставить("ФайлыПоОбластям", Новый Соответствие);
	
	РезультатАнализа.ФайлыПоОбластям.Вставить("kato", АдресЗагрузки);
